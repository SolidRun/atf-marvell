...
#define SCMI_CAVM_CST_PRESRV_REGION_MSG_LEN		16
#define SCMI_CAVM_CST_PRESRV_REGION_RESP_LEN		8
...
{
	uint64_t status = CSR_READ(CAVM_RSTX_COLD_DATAX(1, 1));

	return !(((status >> CAVM_RST_SOURCE_E_XCP0_WDOG) & 1));
}
...
static int is_scp_mcp_running(void)
{
	...
	return !(
		  ((status >> CAVM_RST_SOURCE_E_XCP0_WDOG) & 1) |
		  ((status >> CAVM_RST_SOURCE_E_XCP1_WDOG) & 1)
		);
}
...
static int is_scp_ecp_running(void)
{
	...
	return !(
		  ((status >> CAVM_RST_SOURCE_E_XCP0_WDOG) & 1) |
		  ((status >> CAVM_RST_SOURCE_E_XCP2_WDOG) & 1)
		);
}
