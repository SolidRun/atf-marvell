 ...
 */
#define CAVM_CLKGEN_BAR_E_CLKGEN_PF_BAR0 (0x87e007000000ll)
...
union cavm_clkgen_const
{
    ...
    struct cavm_clkgen_const_s
    {
        ...
        uint64_t runt_bypass           : 1;  /**< [  2:  2](RO/H) When set, The Runt Pulse Oscillator is disabled and hardware always supplies
                                                                 REFCLK during times RUNT clock is normally operating.
                                                                 When clear, software can select runt clock as an output. */
        uint64_t auto_init             : 1;  /**< [  1:  1](RO/H) Auto Hardware Programming.
                                                                 During a reset, ARO/PLLs are always powered down and RUNT Clock is selected
                                                                 as the source.  After that:
                                                                 0 = nothing else happens
                                                                 1 = Hardware programs PLL[INIT_PLL] and then selects it as the clock source
...
{
    if (a<=7)
        return 0x87e007000080ll + 8ll * ((a) & 0x7);
...
{
    if (a<=1)
        return 0x87e007000040ll + 8ll * ((a) & 0x1);
...
union cavm_clkgen_test
{
    ...
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t shift_byp             : 16; /**< [ 63: 48](R/W/H) OCC Shift Bypass.
                                                                 When SHIFT_BYP_SEL is clear, field references CLKOUT OCC Shift Bypass.
                                                                 When set, field references TEST CLKOUT OCC Shift Bypass.
                                                                 Value is only updated on a write if SHIFT_BYP_UPDATE is set.
                                                                 A value of 0 indicates bypass is disabled. */
        uint64_t shift_byp_sel         : 1;  /**< [ 47: 47](R/W) OCC Shift Bypass Selection.  When field is clear, SHIFT_BYP
                                                                 references CLKOUT OCC Shift Bypass.
                                                                 When set, SHIFT_BYP references TEST CLKOUT OCC Shift Bypass. */
        uint64_t shift_byp_update      : 1;  /**< [ 46: 46](WO/H) OCC Shift Bypass Update.  When set, the write will update the
                                                                 selected shift_byp value and hardware will automatically clear the field.
                                                                 When clear, value is maintained. */
        uint64_t reserved_41_45        : 5;
        uint64_t scan_clk_byp          : 1;  /**< [ 40: 40](R/W) When clear, scan_clk and test_scan_clk are directly used by the OCC logic.
                                                                 When set, scan_clk and test_scan_clk going to the OCCs are bypassed by the free
                                                                 running reference clock (pll__ref).
                                                                 Both scan_clk and test_scan_clk must be low when this bit is changed. */
