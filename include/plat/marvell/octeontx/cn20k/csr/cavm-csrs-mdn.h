...
union cavm_mdn_bist_config
{
    ...
    struct cavm_mdn_bist_config_s
    {
        ...
        uint32_t read_disturb_march    : 1;  /**< [ 18: 18](R/W) Setting this bit disables the Read Disturb march. Enable it for specific march
                                                                 sequences required for
                                                                 read disturb kind of testing. This will be used like the Read/Write checkered board marches */
...
union cavm_mdn_ecc_config
{
    ...
    struct cavm_mdn_ecc_config_s
    {
        ...
        uint32_t report_disable        : 1;  /**< [  5:  5](R/W) This disables the reporting of single and double-bit ECC errors and overrides the [DISABLE_SBE] and
...
/**
 * Register (MDSB) mdn_scd_const
 *
 * SCD Constant Register
 */
union cavm_mdn_scd_const
{
    uint32_t u;
    struct cavm_mdn_scd_const_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint32_t reserved_5_31         : 27;
        uint32_t int_red               : 1;  /**< [  4:  4](RO/H) Asserted when Internal Redundancy is used */
        uint32_t mem_bist_mux          : 1;  /**< [  3:  3](RO/H) Asserted when Memory Bist MUX(internal mux) is used */
        uint32_t dbg_rd_disabled       : 1;  /**< [  2:  2](RO/H) Asserted when Debug Reads are disabled for memory */
        uint32_t dbg_wr_disabled       : 1;  /**< [  1:  1](RO/H) Asserted when Debug Reads are disabled for memory */
        uint32_t scrub_disabled        : 1;  /**< [  0:  0](RO/H) Asserted when Scrubbing is disabled for memory */
#else /* Word 0 - Little Endian */
        uint32_t scrub_disabled        : 1;  /**< [  0:  0](RO/H) Asserted when Scrubbing is disabled for memory */
        uint32_t dbg_wr_disabled       : 1;  /**< [  1:  1](RO/H) Asserted when Debug Reads are disabled for memory */
        uint32_t dbg_rd_disabled       : 1;  /**< [  2:  2](RO/H) Asserted when Debug Reads are disabled for memory */
    ...
    /* struct cavm_mdn_scd_const_s cn; */
};
typedef union cavm_mdn_scd_const cavm_mdn_scd_const_t;

#define CAVM_MDN_SCD_CONST CAVM_MDN_SCD_CONST_FUNC()
...
#define bustype_CAVM_MDN_SCD_CONST CSR_TYPE_MDSB
#define basename_CAVM_MDN_SCD_CONST "MDN_SCD_CONST"
#define busnum_CAVM_MDN_SCD_CONST 0
#define arguments_CAVM_MDN_SCD_CONST -1,-1,-1,-1

/**
 * Register (MDSB) mdn_scd_status
 *
 * MDN SCD Status Register
 */
union cavm_mdn_scd_status
{
    uint32_t u;
    struct cavm_mdn_scd_status_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint32_t reserved_2_31         : 30;
        uint32_t scrub_pending         : 1;  /**< [  1:  1](RO/H) Asserted when Scrub is pending */
        uint32_t dbg_pending           : 1;  /**< [  0:  0](RO/H) Asserted when either of Functional Diagnostic Reads or Writes are happening */
#else /* Word 0 - Little Endian */
        uint32_t dbg_pending           : 1;  /**< [  0:  0](RO/H) Asserted when either of Functional Diagnostic Reads or Writes are happening */
        uint32_t scrub_pending         : 1;  /**< [  1:  1](RO/H) Asserted when Scrub is pending */
        uint32_t reserved_2_31         : 30;
    ...
    /* struct cavm_mdn_scd_status_s cn; */
};
typedef union cavm_mdn_scd_status cavm_mdn_scd_status_t;

#define CAVM_MDN_SCD_STATUS CAVM_MDN_SCD_STATUS_FUNC()
...
#define bustype_CAVM_MDN_SCD_STATUS CSR_TYPE_MDSB
#define basename_CAVM_MDN_SCD_STATUS "MDN_SCD_STATUS"
#define busnum_CAVM_MDN_SCD_STATUS 0
#define arguments_CAVM_MDN_SCD_STATUS -1,-1,-1,-1
