...
/**
 * Enumeration pdec_bar_e
 */
#define CAVM_PDEC_BAR_E_PDECX_PF_BAR2(a) (0x87e041900000ll + 0x80000ll * (a))
#define CAVM_PDEC_BAR_E_PDECX_PF_BAR2_SIZE 0x40000ull
...
union cavm_pdec_common_cfg_s
{
    ...
    struct cavm_pdec_common_cfg_s_s
    {
        ...
        uint64_t num_cfg_words_per_task : 8; /**< [ 23: 16] The number of job configuration words for each task. This field must be programed to 0xC. */
...
union cavm_pdec_task_cfg_s
{
    ...
    struct cavm_pdec_task_cfg_s_s
    {
        ...
        uint64_t npc_wm                : 2;  /**< [ 62: 61] Number of parity check bits with minimum row weight, valid for UL only.
                                                                 ...
                                                                 _ if 0xC \<= payload size \<= 0x13 : [NPC_WM] must be in {0x0,0x1}.

                                                                 _ if 0x14 \<= payload size : [NPC_WM] must be 0x0. */
        uint64_t npc                   : 3;  /**< [ 60: 58] Number of parity check bits, valid for UL only.

                                                                 _ if 0xC \<= payload size \<= 0x13, [NPC] must be in {0x0,0x1,0x2,0x3}.

                                                                 _ if 0x14 \<= payload size : [NPC] must be 0x0. */
        ...
        uint64_t enable_reenc          : 1;  /**< [105:105] Enable re-encoding:
                                                                 ...
                                                                 Note that when e_size=0 in UL, enable_reenc must be 0. */
        ...
        uint64_t num_padding           : 3;  /**< [101: 99] Number of padding LLRs that are present in the received input stream.
                                                                 ...
                                                                 Note that [0,1] is sufficient for current standard[R2]
                                                                 Range of [2:7] is for the future extension with the following constraints:
                                                                 When reeenc_qm_with_crc = 0, num_padding shall be in range [0,7].
                                                                 When reeenc_qm_with_crc = 1 and mod_order = 1, num_padding shall be 0 or 1.
                                                                 When reeenc_qm_with_crc = 1 and mod_order \> 1 and C = 1, num_padding shall be in range [0,7].
                                                                 When reeenc_qm_with_crc = 1 and mod_order \> 1 and C = 2, num_padding shall be in
                                                                 range [0, mod_order - 1] */
        uint64_t num_fillers           : 3;  /**< [ 98: 96] The number of filler bits to be removed after decoding.
                                                                 range=[0:7] for UL;
                                                                 range=[0]for DL
                                                                 When num_cbs = 1, num_fillers bits from the beginning of the output stream are removed.
                                                                 When num_cbs = 2, num_fillers bits from the beginning of the output stream
                                                                 (first bits of the first CB) are removed. */
        ...
        uint64_t bypass_intlv          : 1;  /**< [ 79: 79] Bypass de-interleaving.
                                                                 ...
                                                                 This field is only applicable when num_cbs=1. Set to 0 when num_cbs=2.
                                                                 For DL, bypass_intlv=1 */
        uint64_t bypass_rm             : 1;  /**< [ 78: 78] Switch for bypassing de-rate matching.
                                                                 0 = do not bypass rate dematching.
                                                                 1 = bypass rate dematching.

                                                                 bypass_rm only allowed with mod_order=1.
                                                                 num_padding must be 0 when bypass_rm applies to num_cbs=2.

                                                                 UL only. bypass_rm=0 for DL. */
        uint64_t e_size                : 14; /**< [ 77: 64] Number of input LLRs for one CB. Must be either 0x0 or in the range [0x12, 0x2000].
                                                                 ...
                                                                 Note that when e_size=0 in UL, both hcin_en=0 and hcout_en=0; */
#else /* Word 1 - Little Endian */
        uint64_t e_size                : 14; /**< [ 77: 64] Number of input LLRs for one CB. Must be either 0x0 or in the range [0x12, 0x2000].
        ...
        uint64_t bypass_decoder        : 1;  /**< [224:224] 1: perform harq only. Polar decoding is disabled.
                                                                 ...
                                                                 When bypass_decoder=1, no decoded data and report is output and enable_reenc must be set to 0
                                                                 UL only; in DL, decoder always enabled */
        ...
        uint64_t hcout_en              : 1;  /**< [196:196] 1: HCout vector is released after HARQ combining.
                                                                 ...
                                                                 hcout_en = 1 when bypass_decoder = 1

                                                                 Valid range [0:1]

                                                                 Note that when e_size=0 in UL, hcout_en must be 0.
        ...
        uint64_t hcin_en               : 1;  /**< [192:192] 0: Only input LLRs are expected to be received. HARQ is either disabled or this
                                                                 ...
                                                                 Note that when e_size=0 in UL, hcin_en must be 0. */
#else /* Word 3 - Little Endian */
        uint64_t hcin_en               : 1;  /**< [192:192] 0: Only input LLRs are expected to be received. HARQ is either disabled or this
                                                                 is the first transmission.
                                                                 1: HARQ input is enabled. The core expects to receive HCin stream once all input LLRs are received.
...
/**
 * Register (RSL) pdec#_ab#_control
 *
 * PDEC Control Register
 */
union cavm_pdecx_abx_control
{
    uint64_t u;
    struct cavm_pdecx_abx_control_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_control_s cn; */
};
typedef union cavm_pdecx_abx_control cavm_pdecx_abx_control_t;
...
#define typedef_CAVM_PDECX_ABX_CONTROL(a,b) cavm_pdecx_abx_control_t
#define bustype_CAVM_PDECX_ABX_CONTROL(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_CONTROL(a,b) "PDECX_ABX_CONTROL"
#define device_bar_CAVM_PDECX_ABX_CONTROL(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_CONTROL(a,b) (a)
#define arguments_CAVM_PDECX_ABX_CONTROL(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_error_enable0
 *
 * PDEC Error Enable 0 Register
 * This register enables error reporting for read overflow/underflow errors.
 */
union cavm_pdecx_abx_error_enable0
{
    uint64_t u;
    struct cavm_pdecx_abx_error_enable0_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_error_enable0_s cn; */
};
typedef union cavm_pdecx_abx_error_enable0 cavm_pdecx_abx_error_enable0_t;
...
#define typedef_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) cavm_pdecx_abx_error_enable0_t
#define bustype_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) "PDECX_ABX_ERROR_ENABLE0"
#define device_bar_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) (a)
#define arguments_CAVM_PDECX_ABX_ERROR_ENABLE0(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_error_enable1
 *
 * PDEC Error Enable 1 Register
 * This register enables error reporting of ECC errors.
 */
union cavm_pdecx_abx_error_enable1
{
    uint64_t u;
    struct cavm_pdecx_abx_error_enable1_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_error_enable1_s cn; */
};
typedef union cavm_pdecx_abx_error_enable1 cavm_pdecx_abx_error_enable1_t;
...
#define typedef_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) cavm_pdecx_abx_error_enable1_t
#define bustype_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) "PDECX_ABX_ERROR_ENABLE1"
#define device_bar_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) (a)
#define arguments_CAVM_PDECX_ABX_ERROR_ENABLE1(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_error_source0
 *
 * PDEC Error Source 0 Register
 * This register indicates the source of read overflow/underflow errors.
 */
union cavm_pdecx_abx_error_source0
{
    uint64_t u;
    struct cavm_pdecx_abx_error_source0_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_error_source0_s cn; */
};
typedef union cavm_pdecx_abx_error_source0 cavm_pdecx_abx_error_source0_t;
...
#define typedef_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) cavm_pdecx_abx_error_source0_t
#define bustype_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) "PDECX_ABX_ERROR_SOURCE0"
#define device_bar_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) (a)
#define arguments_CAVM_PDECX_ABX_ERROR_SOURCE0(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_error_source1
 *
 * PDEC Error Source 1 Register
 * This register reports the source of ECC errors.
 */
union cavm_pdecx_abx_error_source1
{
    uint64_t u;
    struct cavm_pdecx_abx_error_source1_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_7_63         : 57;
    ...
    /* struct cavm_pdecx_abx_error_source1_s cn; */
};
typedef union cavm_pdecx_abx_error_source1 cavm_pdecx_abx_error_source1_t;
...
#define typedef_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) cavm_pdecx_abx_error_source1_t
#define bustype_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) "PDECX_ABX_ERROR_SOURCE1"
#define device_bar_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) (a)
#define arguments_CAVM_PDECX_ABX_ERROR_SOURCE1(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_hab_jcfg0_ram#_data
 *
...
{
    uint64_t u;
    struct cavm_pdecx_abx_hab_jcfg0_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_hab_jcfg0_ramx_data_s cn; */
};
typedef union cavm_pdecx_abx_hab_jcfg0_ramx_data cavm_pdecx_abx_hab_jcfg0_ramx_data_t;
...
#define typedef_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) cavm_pdecx_abx_hab_jcfg0_ramx_data_t
#define bustype_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) "PDECX_ABX_HAB_JCFG0_RAMX_DATA"
#define device_bar_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PDECX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) pdec#_ab#_hab_jcfg1_ram#_data
 *
 * PDEC HAB Job Configuration 1 RAM Register
 * This register range stores the job configuration for slot 1.
 */
union cavm_pdecx_abx_hab_jcfg1_ramx_data
{
    uint64_t u;
    struct cavm_pdecx_abx_hab_jcfg1_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_hab_jcfg1_ramx_data_s cn; */
};
typedef union cavm_pdecx_abx_hab_jcfg1_ramx_data cavm_pdecx_abx_hab_jcfg1_ramx_data_t;
...
#define typedef_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) cavm_pdecx_abx_hab_jcfg1_ramx_data_t
#define bustype_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) "PDECX_ABX_HAB_JCFG1_RAMX_DATA"
#define device_bar_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PDECX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) pdec#_ab#_hab_jcfg2_ram#_data
 *
 * PDEC HAB Job Configuration 2 RAM Register
 * This register range stores the job configuration for slot 2.
 */
union cavm_pdecx_abx_hab_jcfg2_ramx_data
{
    uint64_t u;
    struct cavm_pdecx_abx_hab_jcfg2_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_hab_jcfg2_ramx_data_s cn; */
};
typedef union cavm_pdecx_abx_hab_jcfg2_ramx_data cavm_pdecx_abx_hab_jcfg2_ramx_data_t;
...
#define typedef_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) cavm_pdecx_abx_hab_jcfg2_ramx_data_t
#define bustype_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) "PDECX_ABX_HAB_JCFG2_RAMX_DATA"
#define device_bar_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PDECX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) pdec#_ab#_status
 *
 * PDEC Status Register
 */
union cavm_pdecx_abx_status
{
    uint64_t u;
    struct cavm_pdecx_abx_status_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_status_s cn; */
};
typedef union cavm_pdecx_abx_status cavm_pdecx_abx_status_t;
...
#define typedef_CAVM_PDECX_ABX_STATUS(a,b) cavm_pdecx_abx_status_t
#define bustype_CAVM_PDECX_ABX_STATUS(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_STATUS(a,b) "PDECX_ABX_STATUS"
#define device_bar_CAVM_PDECX_ABX_STATUS(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_STATUS(a,b) (a)
#define arguments_CAVM_PDECX_ABX_STATUS(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_cfg_err_flags_reg
 *
 ...
 */
union cavm_pdecx_abx_tc_cfg_err_flags_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_cfg_err_flags_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_tc_cfg_err_flags_reg_s cn; */
};
typedef union cavm_pdecx_abx_tc_cfg_err_flags_reg cavm_pdecx_abx_tc_cfg_err_flags_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) cavm_pdecx_abx_tc_cfg_err_flags_reg_t
#define bustype_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) "PDECX_ABX_TC_CFG_ERR_FLAGS_REG"
#define device_bar_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_CFG_ERR_FLAGS_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_control_reg
 *
 * PDEC Decoder Control Register
 * This register configures the PDEC internal decoder core.
 */
union cavm_pdecx_abx_tc_control_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_control_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_5_63         : 59;
    ...
    struct cavm_pdecx_abx_tc_control_reg_cn
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
...
typedef union cavm_pdecx_abx_tc_control_reg cavm_pdecx_abx_tc_control_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) cavm_pdecx_abx_tc_control_reg_t
#define bustype_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) "PDECX_ABX_TC_CONTROL_REG"
#define device_bar_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_CONTROL_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_error_mask_reg
 *
 ...
 */
union cavm_pdecx_abx_tc_error_mask_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_error_mask_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_32_63        : 32;
    ...
    /* struct cavm_pdecx_abx_tc_error_mask_reg_s cn; */
};
typedef union cavm_pdecx_abx_tc_error_mask_reg cavm_pdecx_abx_tc_error_mask_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) cavm_pdecx_abx_tc_error_mask_reg_t
#define bustype_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) "PDECX_ABX_TC_ERROR_MASK_REG"
#define device_bar_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_ERROR_MASK_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_error_reg
 *
 ...
 * the error(s).
 */
union cavm_pdecx_abx_tc_error_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_error_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_tc_error_reg_s cn; */
};
typedef union cavm_pdecx_abx_tc_error_reg cavm_pdecx_abx_tc_error_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) cavm_pdecx_abx_tc_error_reg_t
#define bustype_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) "PDECX_ABX_TC_ERROR_REG"
#define device_bar_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_ERROR_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_main_reset_reg
 *
 * PDEC Decoder Reset Register
 * Internal decoder core reset register.
 */
union cavm_pdecx_abx_tc_main_reset_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_main_reset_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_tc_main_reset_reg_s cn; */
};
typedef union cavm_pdecx_abx_tc_main_reset_reg cavm_pdecx_abx_tc_main_reset_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) cavm_pdecx_abx_tc_main_reset_reg_t
#define bustype_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) "PDECX_ABX_TC_MAIN_RESET_REG"
#define device_bar_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_MAIN_RESET_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) pdec#_ab#_tc_status_reg
 *
 * PDEC Decoder Status Register
 * This register reports status from the internal decoder core.
 */
union cavm_pdecx_abx_tc_status_reg
{
    uint64_t u;
    struct cavm_pdecx_abx_tc_status_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pdecx_abx_tc_status_reg_s cn; */
};
typedef union cavm_pdecx_abx_tc_status_reg cavm_pdecx_abx_tc_status_reg_t;
...
#define typedef_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) cavm_pdecx_abx_tc_status_reg_t
#define bustype_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) "PDECX_ABX_TC_STATUS_REG"
#define device_bar_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) 0x2 /* PF_BAR2 */
#define busnum_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) (a)
#define arguments_CAVM_PDECX_ABX_TC_STATUS_REG(a,b) (a),(b),-1,-1
