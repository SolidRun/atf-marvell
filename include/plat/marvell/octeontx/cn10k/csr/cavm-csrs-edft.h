...
union cavm_edft_common_cfg_s
{
    ...
    struct cavm_edft_common_cfg_s_s
    {
        ...
        uint64_t num_tot_cfg_words     : 6;  /**< [ 55: 50] Number of configuration 64b words (including this common configuration word):
                                                                 For TASK_TYPE=0, NUM_TOT_CFG_WORDS=2
                                                                 For TASK_TYPE=1, if RS_COMB_ENABLE=1, NUM_TOT_CFG_WORDS=6, else NUM_TOT_CFG_WORDS=6+NUM_STREAMS/2
        ...
        uint64_t dft_idft_select       : 1;  /**< [ 32: 32] 0 = this block operates as an IDFT.
                                                                 1 = this block operates as a DFT. */
        uint64_t reserved_30_31        : 2;
        uint64_t idft_size_idx         : 6;  /**< [ 29: 24] 0 = this block operates as an IDFT.
                                                                 1 = this block operates as a DFT. */
        uint64_t reserved_21_23        : 3;
        uint64_t dft_rs_last           : 5;  /**< [ 20: 16] 0 = this block operates as an IDFT.
                                                                 1 = this block operates as a DFT. */
...
union cavm_edft_dmrs_cfg_s
{
    ...
    struct cavm_edft_dmrs_cfg_s_s
    {
        ...
        uint64_t length                : 11; /**< [ 58: 48] Number of 32b sample output of each sequence generator module
                                                                 Total number of RS symbols: 2 x length, range 1-1650. */
        ...
        uint64_t signature             : 31; /**< [ 42: 12] c_init for base_seq 1, 2
                                                                 ...
                                                                 Nzc should be a prime number less than Mzc (sec 5.2.2.1 in REF1.) */
        uint64_t start_sample_offset   : 12; /**< [ 11:  0] start sample offset. Skip the first start_sample_offset samples. (valid range 0-3243)
                                                                 Use case a: Multi-cluster case where the largest cluster based on which Time
                                                                 offset is calculated is not the first cluster.
                                                                 Use Case b: PRS where the first sample used for this task needs to be adjusted
                                                                 based on the start_rb.
                                                                 Note: This offset specifies the number of DMRS tones skipped and not the number of REs skipped. */
#else /* Word 0 - Little Endian */
        uint64_t start_sample_offset   : 12; /**< [ 11:  0] start sample offset. Skip the first start_sample_offset samples. (valid range 0-3243)
                                                                 Use case a: Multi-cluster case where the largest cluster based on which Time
                                                                 offset is calculated is not the first cluster.
...
union cavm_edft_toe_cfg_s
{
    ...
    struct cavm_edft_toe_cfg_s_s
    {
        ...
        uint64_t wt_1                  : 1;  /**< [ 60: 60] WT_1 for OCC1 block */
        uint64_t wf_1                  : 1;  /**< [ 59: 59] WF_1 for OCC1 block */
        ...
        uint64_t left_win_size         : 10; /**< [109:100] Size (in samples) of left window for peak search. */
        uint64_t reserved_98_99        : 2;
        uint64_t right_win_size        : 10; /**< [ 97: 88] Size (in samples) of right window for peak search. */
        uint64_t reserved_86_87        : 2;
        uint64_t w_ni_start            : 10; /**< [ 85: 76] Sample index of start of the window searching for noise peak. */
        uint64_t reserved_74_75        : 2;
        uint64_t w_ni_size             : 10; /**< [ 73: 64] Size, in number of samples, of the window searching for noise peak. */
#else /* Word 1 - Little Endian */
        uint64_t w_ni_size             : 10; /**< [ 73: 64] Size, in number of samples, of the window searching for noise peak. */
        uint64_t reserved_74_75        : 2;
        uint64_t w_ni_start            : 10; /**< [ 85: 76] Sample index of start of the window searching for noise peak. */
    ...
    /* struct cavm_edft_toe_cfg_s_s cn; */
};

/**
 * Register (RSL) edft#_ab#_bp_test0
 *
 * INTERNAL: EDFT Back Pressure Test 0 Register
 *
 * This register is used to stress internal Hardware backpressure points for Testing
 */
union cavm_edftx_abx_bp_test0
{
    uint64_t u;
    struct cavm_edftx_abx_bp_test0_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_edftx_abx_bp_test0_s cn; */
};
typedef union cavm_edftx_abx_bp_test0 cavm_edftx_abx_bp_test0_t;

static inline uint64_t CAVM_EDFTX_ABX_BP_TEST0(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_EDFTX_ABX_BP_TEST0(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=1))
        return 0x87e041c00020ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x1);
    __cavm_csr_fatal("EDFTX_ABX_BP_TEST0", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_EDFTX_ABX_BP_TEST0(a,b) cavm_edftx_abx_bp_test0_t
#define bustype_CAVM_EDFTX_ABX_BP_TEST0(a,b) CSR_TYPE_RSL
#define basename_CAVM_EDFTX_ABX_BP_TEST0(a,b) "EDFTX_ABX_BP_TEST0"
#define busnum_CAVM_EDFTX_ABX_BP_TEST0(a,b) (a)
#define arguments_CAVM_EDFTX_ABX_BP_TEST0(a,b) (a),(b),-1,-1
...
union cavm_edftx_abx_control
{
    ...
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t jobid2                : 16; /**< [ 63: 48](R/W) Job ID for slot 2. */
...
/**
 * Register (RSL) edft#_ab#_eco
 *
 * INTERNAL: EDFT ECO Register
 *
 * An ECO CSR
 */
union cavm_edftx_abx_eco
{
    uint64_t u;
    struct cavm_edftx_abx_eco_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_edftx_abx_eco_s cn; */
};
typedef union cavm_edftx_abx_eco cavm_edftx_abx_eco_t;

static inline uint64_t CAVM_EDFTX_ABX_ECO(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_EDFTX_ABX_ECO(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=1))
        return 0x87e041c00010ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x1);
    __cavm_csr_fatal("EDFTX_ABX_ECO", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_EDFTX_ABX_ECO(a,b) cavm_edftx_abx_eco_t
#define bustype_CAVM_EDFTX_ABX_ECO(a,b) CSR_TYPE_RSL
#define basename_CAVM_EDFTX_ABX_ECO(a,b) "EDFTX_ABX_ECO"
#define busnum_CAVM_EDFTX_ABX_ECO(a,b) (a)
#define arguments_CAVM_EDFTX_ABX_ECO(a,b) (a),(b),-1,-1
...
/**
 * Register (RSL) edft#_ab#_scratch
 *
 * INTERNAL: Scratch Register
 *
 * Scratch register.
 */
union cavm_edftx_abx_scratch
{
    uint64_t u;
    struct cavm_edftx_abx_scratch_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_edftx_abx_scratch_s cn; */
};
typedef union cavm_edftx_abx_scratch cavm_edftx_abx_scratch_t;

static inline uint64_t CAVM_EDFTX_ABX_SCRATCH(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_EDFTX_ABX_SCRATCH(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=1))
        return 0x87e041c00080ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x1);
    __cavm_csr_fatal("EDFTX_ABX_SCRATCH", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_EDFTX_ABX_SCRATCH(a,b) cavm_edftx_abx_scratch_t
#define bustype_CAVM_EDFTX_ABX_SCRATCH(a,b) CSR_TYPE_RSL
#define basename_CAVM_EDFTX_ABX_SCRATCH(a,b) "EDFTX_ABX_SCRATCH"
#define busnum_CAVM_EDFTX_ABX_SCRATCH(a,b) (a)
#define arguments_CAVM_EDFTX_ABX_SCRATCH(a,b) (a),(b),-1,-1
