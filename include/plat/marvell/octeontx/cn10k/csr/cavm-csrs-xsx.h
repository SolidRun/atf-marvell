...
union cavm_xsx_bp_test
{
    ...
    struct cavm_xsx_bp_test_s
    {
        ...
        uint64_t enable                : 4;  /**< [ 63: 60](R/W) Enable test mode. For diagnostic use only.
                                                                 ...
                                                                 \<63\> = xbar1 responses (odd XBT tile)
                                                                 \<62\> = xbar1 responses (even XBT tile)
                                                                 \<61\> = xbar0 responses (odd XBT tile)
                                                                 \<60\> = xbar0 responses (even XBT tile) */
        ...
        uint64_t bp_cfg                : 8;  /**< [ 23: 16](R/W) Backpressure weight. For diagnostic use only.
                                                                 ...
                                                                 0x3=25% of the time.
                                                                 \<23:22\> = xbar1 responses (odd XBT tile)
                                                                 \<21:20\> = xbar1 responses (even XBT tile)
                                                                 \<19:18\> = xbar0 responses (odd XBT tile)
                                                                 \<17:16\> = xbar0 responses (even XBT tile) */
...
union cavm_xsx_control
{
    ...
    struct cavm_xsx_control_s
    {
        ...
        uint64_t smt_clk_disable       : 16; /**< [ 23:  8](R/W) Disable the main SMT clock. Each bit controls the clock for one SMT. */
        uint64_t reserved_6_7          : 2;
        uint64_t smem_active_size      : 2;  /**< [  5:  4](R/W) Select size of SMEM.  smem_active_size defaults to 3 and should be programmed
                                                                 after RESET, before any SMEM memory accesses are made. Note that the unused
                                                                 SRAM macros are shutdown in order to save power, and their contents are lost.
                                                                   3 = 128 active banks (4MByte per SMT)
                                                                   2 = 96 active banks  (3MByte per SMT)
                                                                   1 = 64 active banks  (2MByte per SMT)
                                                                   0 = 32 active banks  (1MByte per SMT)
...
{
    if ((a<=15) && (b<=1))
        return 0x87e043030240ll + 0x400ll * ((a) & 0xf) + 8ll * ((b) & 0x1);
 ...
 * one to the SBE and/or DBE bit will clear the error.
 */
union cavm_xsx_smtx_err
{
    ...
    struct cavm_xsx_smtx_err_s
    {
        ...
        uint64_t addr                  : 17; /**< [ 32: 16](RO/H) SMT Address of most recent error transaction */
        uint64_t reserved_13_15        : 3;
        uint64_t ghbid                 : 5;  /**< [ 12:  8](RO/H) GHB/GDB ID of most recent error transaction */
        uint64_t reserved_7            : 1;
        uint64_t mhbid                 : 3;  /**< [  6:  4](RO/H) MHB/MDB ID of most recent error transaction */
        uint64_t xbar                  : 1;  /**< [  3:  3](RO/H) XBAR0/XBAR1 port of most recent error transaction */
        uint64_t rnw                   : 1;  /**< [  2:  2](RO/H) RnW of most recent error transaction */
        uint64_t dbe                   : 1;  /**< [  1:  1](R/W1C/H) Uncorrectable double bit error(s) */
        uint64_t sbe                   : 1;  /**< [  0:  0](R/W1C/H) Correctable single bit error(s) */
#else /* Word 0 - Little Endian */
        uint64_t sbe                   : 1;  /**< [  0:  0](R/W1C/H) Correctable single bit error(s) */
        uint64_t dbe                   : 1;  /**< [  1:  1](R/W1C/H) Uncorrectable double bit error(s) */
        uint64_t rnw                   : 1;  /**< [  2:  2](RO/H) RnW of most recent error transaction */
 ...
 * weights are programmed separately for each of the 16 memory tiles
 * (SMT(0..15)).
 *
 ...
 * Number of read request. *xbar0: ports 0-15 are GHB 0-15; port 16 is PNB. *xbar1: ports 0-15 are
 * GDB 0-15; port16 is PNB.
 */
 ...
 * Number of write request. *xbar0: ports 0-15 are GHB 0-15; port 16 is PNB. *xbar1: ports 0-15
 * are GDB 0-15; port16 is PNB.
 */
