...
union cavm_penc_common_cfg_s
{
    ...
    struct cavm_penc_common_cfg_s_s
    {
        ...
        uint64_t num_cfg_words_per_task : 8; /**< [ 31: 24] Number of configuration words per task. Must be 0xA for all PENC jobs. */
...
union cavm_penc_task_cfg_s
{
    ...
    struct cavm_penc_task_cfg_s_s
    {
        ...
        uint64_t g_size                : 14; /**< [ 45: 32] Size after rate matching (corresponds to G in 3GPP).
        ...
        uint64_t dl0ul1                : 1;  /**< [ 24: 24] Selects Downlink or Uplink modes of operation.
                                                                 0 = DL mode.
                                                                 1 = UL mode. */
        ...
        uint64_t reserved_310_319      : 10;
        uint64_t ul_bypass_rm          : 1;  /**< [309:309] 0 = not bypassed
                                                                 1 = bypass rate matching,
                                                                 When [UL_BYPASS_RM] =1, the output size is the mother code length, which is
                                                                 determined by ul_k_size and ul_e_size
                                                                 [UL_BYPASS_RM] can only be 1 when [NUM_CBS]=1.
                                                                 Used in UL mode only.  When [DL0UL1]=0, [UL_BYPASS_RM] must be 0.
                                                                 Scrambling and symbol byte aligned (symb_aligned shall be 0) are also bypassed
                                                                 when [UL_BYPASS_RM] is 1.
                                                                 When [UL_BYPASS_RM] =1, [UL_NUM_PADDING] must be 0 and [BYPASS_SEGMENTATION]
                                                                 must be 1 (no segmentation). */
        uint64_t reserved_308          : 1;
        uint64_t ul_crc_select         : 2;  /**< [307:306] Selects the CRC polynomial for UL mode.
                                                                 The same CRC is used for both CBs when num_cbs = 2.
                                                                 0x0 = polynomial gCRC6
                                                                 0x1 = polynomial gCRC11
                                                                 0x2-0x3 = reserved
                                                                 Used in UL mode only (when [DL0UL1]=1). */
        uint64_t ul_num_padding        : 1;  /**< [305:305] The number of padding bits after encoding.
                                                                 Range=[0,1]
                                                                 num_padding bits are added at the end of bit stream.
                                                                 Used in UL mode only (when [DL0UL1]=1).
                                                                 Must be 0 when [UL_BYPASS_RM] is 1. */
        uint64_t ul_num_fillers        : 1;  /**< [304:304] The number of filler bits to be added before encoding.
                                                                 Range=[0,1]
                                                                 When [UL_NUM_CBS] = 0x1, num_fillers bits from the beginning of the output stream are added.
                                                                 When [UL_NUM_CBS] = 0x2, num_fillers bits from the beginning of the output
                                                                 stream (first bits of the first CB) are added.
                                                                 Used in UL mode only (when [DL0UL1]=1).
                                                                 Must be 0 when [UL_BYPASS_CRC] is 1. */
        uint64_t ul_bypass_tri_intlv   : 1;  /**< [303:303] UL: bypass triangular interleaver.
                                                                 Used in UL mode only (when [DL0UL1]=1). */
        uint64_t reserved_284_302      : 19;
        uint64_t ul_num_cbs            : 2;  /**< [283:282] Number of CBs after CB segmentation.
                                                                 Used in UL mode only (when [DL0UL1]=1).
                                                                 Range=[1,2]. */
        uint64_t ul_e_size             : 14; /**< [281:268] Number of output bits for one CB after rate-matching.
                                                                 e_size shall be always \>= (k_size + npc) to ensure code rate \>= 1.0.
                                                                 The total number of the output stream is G = num_cbs * e_size + num_padding,
                                                                 where G corresponds to the parameter G in 38.212).
                                                                 Used in UL mode only (when [DL0UL1]=1).
                                                                 Range=[k_size+npc, 8192] where npc = 3 for k \<= 25. */
        uint64_t ul_k_size             : 12; /**< [267:256] UL Payload block size including CRC for one CB.
                                                                 The length of the input stream is: A = (k_size - crc_len) * num_cbs -
                                                                 num_fillers, where A corresponds to the parameter A in 38.212 spec.
                                                                 Used in UL mode only (when [DL0UL1]=1).
                                                                 Range =[18,1024] when [BYPASS_CRC] = 0 else [1,1024] when [BYPASS_CRC] = 1. */
        ...
        uint64_t num_seg3_sym          : 16; /**< [383:368] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg2_sym          : 16; /**< [367:352] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg1_sym          : 16; /**< [351:336] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg0_sym          : 16; /**< [335:320] See [SEGMENT_PADDING_FLAG]. */
        ...
        uint64_t num_seg7_sym          : 16; /**< [447:432] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg6_sym          : 16; /**< [431:416] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg5_sym          : 16; /**< [415:400] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg4_sym          : 16; /**< [399:384] See [SEGMENT_PADDING_FLAG]. */
        ...
        uint64_t num_seg11_sym         : 16; /**< [511:496] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg10_sym         : 16; /**< [495:480] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg9_sym          : 16; /**< [479:464] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg8_sym          : 16; /**< [463:448] See [SEGMENT_PADDING_FLAG]. */
        ...
        uint64_t reserved_570_575      : 6;
        uint64_t num_total_sym         : 24; /**< [569:546] When [SEGMENT_PADDING_FLAG] = 1, specifies the total number of symbols in all
        ...
        uint64_t segment_padding_flag  : 1;  /**< [545:545] Segment padding mode:
        ...
        uint64_t bypass_segmentation   : 1;  /**< [544:544] Used only when ([CH_SELECTION] = 0x0 or [CH_SELECTION] = 0x1); otherwise, set the value to 1.
        ...
        uint64_t num_seg13_sym         : 16; /**< [543:528] See [SEGMENT_PADDING_FLAG]. */
        uint64_t num_seg12_sym         : 16; /**< [527:512] See [SEGMENT_PADDING_FLAG]. */
        ...
        uint64_t num_wr0_dma_words     : 32; /**< [639:608] Number of words to write from write DMA port 0 for this task. */
        uint64_t num_rd0_dma_words     : 32; /**< [607:576] Number of words to read from read DMA port 0 for this task. */
...
/**
 * Register (RSL) penc#_ab#_control
 *
 * PENC Control Register
 */
union cavm_pencx_abx_control
{
    uint64_t u;
    struct cavm_pencx_abx_control_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_control_s cn; */
};
typedef union cavm_pencx_abx_control cavm_pencx_abx_control_t;
...
#define typedef_CAVM_PENCX_ABX_CONTROL(a,b) cavm_pencx_abx_control_t
#define bustype_CAVM_PENCX_ABX_CONTROL(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_CONTROL(a,b) "PENCX_ABX_CONTROL"
#define busnum_CAVM_PENCX_ABX_CONTROL(a,b) (a)
#define arguments_CAVM_PENCX_ABX_CONTROL(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_eco
 *
 * INTERNAL: PENC ECO Register
 */
union cavm_pencx_abx_eco
{
    uint64_t u;
    struct cavm_pencx_abx_eco_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_32_63        : 32;
    ...
    /* struct cavm_pencx_abx_eco_s cn; */
};
typedef union cavm_pencx_abx_eco cavm_pencx_abx_eco_t;

static inline uint64_t CAVM_PENCX_ABX_ECO(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_PENCX_ABX_ECO(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=2))
        return 0x87e040800008ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x3);
    __cavm_csr_fatal("PENCX_ABX_ECO", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_PENCX_ABX_ECO(a,b) cavm_pencx_abx_eco_t
#define bustype_CAVM_PENCX_ABX_ECO(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_ECO(a,b) "PENCX_ABX_ECO"
#define busnum_CAVM_PENCX_ABX_ECO(a,b) (a)
#define arguments_CAVM_PENCX_ABX_ECO(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_error_enable0
 *
 * Error Enable 0 Register
 * This register enables error reporting for read overflow/underflow errors.
 */
union cavm_pencx_abx_error_enable0
{
    uint64_t u;
    struct cavm_pencx_abx_error_enable0_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_error_enable0_s cn; */
};
typedef union cavm_pencx_abx_error_enable0 cavm_pencx_abx_error_enable0_t;
...
#define typedef_CAVM_PENCX_ABX_ERROR_ENABLE0(a,b) cavm_pencx_abx_error_enable0_t
#define bustype_CAVM_PENCX_ABX_ERROR_ENABLE0(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_ERROR_ENABLE0(a,b) "PENCX_ABX_ERROR_ENABLE0"
#define busnum_CAVM_PENCX_ABX_ERROR_ENABLE0(a,b) (a)
#define arguments_CAVM_PENCX_ABX_ERROR_ENABLE0(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_error_source0
 *
 * PENC Error Source 0 Register
 * This register indicates the source of read overflow/underflow errors.
 */
union cavm_pencx_abx_error_source0
{
    uint64_t u;
    struct cavm_pencx_abx_error_source0_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_32_63        : 32;
    ...
    /* struct cavm_pencx_abx_error_source0_s cn; */
};
typedef union cavm_pencx_abx_error_source0 cavm_pencx_abx_error_source0_t;
...
#define typedef_CAVM_PENCX_ABX_ERROR_SOURCE0(a,b) cavm_pencx_abx_error_source0_t
#define bustype_CAVM_PENCX_ABX_ERROR_SOURCE0(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_ERROR_SOURCE0(a,b) "PENCX_ABX_ERROR_SOURCE0"
#define busnum_CAVM_PENCX_ABX_ERROR_SOURCE0(a,b) (a)
#define arguments_CAVM_PENCX_ABX_ERROR_SOURCE0(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_hab_jcfg0_ram#_data
 *
...
{
    uint64_t u;
    struct cavm_pencx_abx_hab_jcfg0_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_hab_jcfg0_ramx_data_s cn; */
};
typedef union cavm_pencx_abx_hab_jcfg0_ramx_data cavm_pencx_abx_hab_jcfg0_ramx_data_t;
...
#define typedef_CAVM_PENCX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) cavm_pencx_abx_hab_jcfg0_ramx_data_t
#define bustype_CAVM_PENCX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) "PENCX_ABX_HAB_JCFG0_RAMX_DATA"
#define busnum_CAVM_PENCX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PENCX_ABX_HAB_JCFG0_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) penc#_ab#_hab_jcfg1_ram#_data
 *
 * PENC Job Configuration 1 RAM Register
 * This register range stores the job configuration for slot 1.
 */
union cavm_pencx_abx_hab_jcfg1_ramx_data
{
    uint64_t u;
    struct cavm_pencx_abx_hab_jcfg1_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_hab_jcfg1_ramx_data_s cn; */
};
typedef union cavm_pencx_abx_hab_jcfg1_ramx_data cavm_pencx_abx_hab_jcfg1_ramx_data_t;
...
#define typedef_CAVM_PENCX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) cavm_pencx_abx_hab_jcfg1_ramx_data_t
#define bustype_CAVM_PENCX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) "PENCX_ABX_HAB_JCFG1_RAMX_DATA"
#define busnum_CAVM_PENCX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PENCX_ABX_HAB_JCFG1_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) penc#_ab#_hab_jcfg2_ram#_data
 *
 * PENC Job Configuration 2 RAM Register
 * This register range stores the job configuration for slot 2.
 */
union cavm_pencx_abx_hab_jcfg2_ramx_data
{
    uint64_t u;
    struct cavm_pencx_abx_hab_jcfg2_ramx_data_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_hab_jcfg2_ramx_data_s cn; */
};
typedef union cavm_pencx_abx_hab_jcfg2_ramx_data cavm_pencx_abx_hab_jcfg2_ramx_data_t;
...
#define typedef_CAVM_PENCX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) cavm_pencx_abx_hab_jcfg2_ramx_data_t
#define bustype_CAVM_PENCX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) "PENCX_ABX_HAB_JCFG2_RAMX_DATA"
#define busnum_CAVM_PENCX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) (a)
#define arguments_CAVM_PENCX_ABX_HAB_JCFG2_RAMX_DATA(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) penc#_ab#_scratch
 *
 * INTERNAL: Scratch Register
 *
 * Scratch register.
 */
union cavm_pencx_abx_scratch
{
    uint64_t u;
    struct cavm_pencx_abx_scratch_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_scratch_s cn; */
};
typedef union cavm_pencx_abx_scratch cavm_pencx_abx_scratch_t;

static inline uint64_t CAVM_PENCX_ABX_SCRATCH(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_PENCX_ABX_SCRATCH(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=2))
        return 0x87e040800080ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x3);
    __cavm_csr_fatal("PENCX_ABX_SCRATCH", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_PENCX_ABX_SCRATCH(a,b) cavm_pencx_abx_scratch_t
#define bustype_CAVM_PENCX_ABX_SCRATCH(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_SCRATCH(a,b) "PENCX_ABX_SCRATCH"
#define busnum_CAVM_PENCX_ABX_SCRATCH(a,b) (a)
#define arguments_CAVM_PENCX_ABX_SCRATCH(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_status
 *
 * PENC Status Register
 */
union cavm_pencx_abx_status
{
    uint64_t u;
    struct cavm_pencx_abx_status_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_5_63         : 59;
    ...
    /* struct cavm_pencx_abx_status_s cn; */
};
typedef union cavm_pencx_abx_status cavm_pencx_abx_status_t;
...
#define typedef_CAVM_PENCX_ABX_STATUS(a,b) cavm_pencx_abx_status_t
#define bustype_CAVM_PENCX_ABX_STATUS(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_STATUS(a,b) "PENCX_ABX_STATUS"
#define busnum_CAVM_PENCX_ABX_STATUS(a,b) (a)
#define arguments_CAVM_PENCX_ABX_STATUS(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_config_err_flags
 *
 ...
 */
union cavm_pencx_abx_tc_config_err_flags
{
    uint64_t u;
    struct cavm_pencx_abx_tc_config_err_flags_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_config_err_flags_s cn; */
};
typedef union cavm_pencx_abx_tc_config_err_flags cavm_pencx_abx_tc_config_err_flags_t;
...
#define typedef_CAVM_PENCX_ABX_TC_CONFIG_ERR_FLAGS(a,b) cavm_pencx_abx_tc_config_err_flags_t
#define bustype_CAVM_PENCX_ABX_TC_CONFIG_ERR_FLAGS(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_CONFIG_ERR_FLAGS(a,b) "PENCX_ABX_TC_CONFIG_ERR_FLAGS"
#define busnum_CAVM_PENCX_ABX_TC_CONFIG_ERR_FLAGS(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_CONFIG_ERR_FLAGS(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_config_reg#
 *
 * INTERNAL: TC Task Config Registers
 *
 * TC task config registers.
 */
union cavm_pencx_abx_tc_config_regx
{
    uint64_t u;
    struct cavm_pencx_abx_tc_config_regx_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_config_regx_s cn; */
};
typedef union cavm_pencx_abx_tc_config_regx cavm_pencx_abx_tc_config_regx_t;

static inline uint64_t CAVM_PENCX_ABX_TC_CONFIG_REGX(uint64_t a, uint64_t b, uint64_t c) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_PENCX_ABX_TC_CONFIG_REGX(uint64_t a, uint64_t b, uint64_t c)
{
    if ((a<=1) && (b<=2) && (c<=2))
        return 0x87e040801400ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x3) + 8ll * ((c) & 0x3);
    __cavm_csr_fatal("PENCX_ABX_TC_CONFIG_REGX", 3, a, b, c, 0, 0, 0);
}

#define typedef_CAVM_PENCX_ABX_TC_CONFIG_REGX(a,b,c) cavm_pencx_abx_tc_config_regx_t
#define bustype_CAVM_PENCX_ABX_TC_CONFIG_REGX(a,b,c) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_CONFIG_REGX(a,b,c) "PENCX_ABX_TC_CONFIG_REGX"
#define busnum_CAVM_PENCX_ABX_TC_CONFIG_REGX(a,b,c) (a)
#define arguments_CAVM_PENCX_ABX_TC_CONFIG_REGX(a,b,c) (a),(b),(c),-1

/**
 * Register (RSL) penc#_ab#_tc_control_reg
 *
 * PENC Encoder Control Register
 * This register configures the PENC internal encoder core.
 */
union cavm_pencx_abx_tc_control_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_control_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_5_63         : 59;
    ...
    struct cavm_pencx_abx_tc_control_reg_cn
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_32_63        : 32;
...
typedef union cavm_pencx_abx_tc_control_reg cavm_pencx_abx_tc_control_reg_t;
...
#define typedef_CAVM_PENCX_ABX_TC_CONTROL_REG(a,b) cavm_pencx_abx_tc_control_reg_t
#define bustype_CAVM_PENCX_ABX_TC_CONTROL_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_CONTROL_REG(a,b) "PENCX_ABX_TC_CONTROL_REG"
#define busnum_CAVM_PENCX_ABX_TC_CONTROL_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_CONTROL_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_error_mask_reg
 *
 ...
 */
union cavm_pencx_abx_tc_error_mask_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_error_mask_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_32_63        : 32;
    ...
    /* struct cavm_pencx_abx_tc_error_mask_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_error_mask_reg cavm_pencx_abx_tc_error_mask_reg_t;
...
#define typedef_CAVM_PENCX_ABX_TC_ERROR_MASK_REG(a,b) cavm_pencx_abx_tc_error_mask_reg_t
#define bustype_CAVM_PENCX_ABX_TC_ERROR_MASK_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_ERROR_MASK_REG(a,b) "PENCX_ABX_TC_ERROR_MASK_REG"
#define busnum_CAVM_PENCX_ABX_TC_ERROR_MASK_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_ERROR_MASK_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_error_reg
 *
 ...
 * the error(s).
 */
union cavm_pencx_abx_tc_error_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_error_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_error_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_error_reg cavm_pencx_abx_tc_error_reg_t;
...
#define typedef_CAVM_PENCX_ABX_TC_ERROR_REG(a,b) cavm_pencx_abx_tc_error_reg_t
#define bustype_CAVM_PENCX_ABX_TC_ERROR_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_ERROR_REG(a,b) "PENCX_ABX_TC_ERROR_REG"
#define busnum_CAVM_PENCX_ABX_TC_ERROR_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_ERROR_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_main_reset_reg
 *
 * PENC Encoder Reset Register
 * This register provides a reset for the internal encoder core.
 */
union cavm_pencx_abx_tc_main_reset_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_main_reset_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_main_reset_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_main_reset_reg cavm_pencx_abx_tc_main_reset_reg_t;
...
#define typedef_CAVM_PENCX_ABX_TC_MAIN_RESET_REG(a,b) cavm_pencx_abx_tc_main_reset_reg_t
#define bustype_CAVM_PENCX_ABX_TC_MAIN_RESET_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_MAIN_RESET_REG(a,b) "PENCX_ABX_TC_MAIN_RESET_REG"
#define busnum_CAVM_PENCX_ABX_TC_MAIN_RESET_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_MAIN_RESET_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_main_start_reg
 *
 * INTERNAL: TC Start Register
 *
 * TC start register.
 */
union cavm_pencx_abx_tc_main_start_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_main_start_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
        uint64_t reserved_1_63         : 63;
    ...
    /* struct cavm_pencx_abx_tc_main_start_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_main_start_reg cavm_pencx_abx_tc_main_start_reg_t;

static inline uint64_t CAVM_PENCX_ABX_TC_MAIN_START_REG(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_PENCX_ABX_TC_MAIN_START_REG(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=2))
        return 0x87e040801008ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x3);
    __cavm_csr_fatal("PENCX_ABX_TC_MAIN_START_REG", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_PENCX_ABX_TC_MAIN_START_REG(a,b) cavm_pencx_abx_tc_main_start_reg_t
#define bustype_CAVM_PENCX_ABX_TC_MAIN_START_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_MAIN_START_REG(a,b) "PENCX_ABX_TC_MAIN_START_REG"
#define busnum_CAVM_PENCX_ABX_TC_MAIN_START_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_MAIN_START_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_mon_reg
 *
 * INTERNAL: TC Monitoring Registers
 *
 * TC mon registers.
 */
union cavm_pencx_abx_tc_mon_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_mon_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_mon_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_mon_reg cavm_pencx_abx_tc_mon_reg_t;

static inline uint64_t CAVM_PENCX_ABX_TC_MON_REG(uint64_t a, uint64_t b) __attribute__ ((pure, always_inline));
static inline uint64_t CAVM_PENCX_ABX_TC_MON_REG(uint64_t a, uint64_t b)
{
    if ((a<=1) && (b<=2))
        return 0x87e040801300ll + 0x80000ll * ((a) & 0x1) + 0x10000ll * ((b) & 0x3);
    __cavm_csr_fatal("PENCX_ABX_TC_MON_REG", 2, a, b, 0, 0, 0, 0);
}

#define typedef_CAVM_PENCX_ABX_TC_MON_REG(a,b) cavm_pencx_abx_tc_mon_reg_t
#define bustype_CAVM_PENCX_ABX_TC_MON_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_MON_REG(a,b) "PENCX_ABX_TC_MON_REG"
#define busnum_CAVM_PENCX_ABX_TC_MON_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_MON_REG(a,b) (a),(b),-1,-1

/**
 * Register (RSL) penc#_ab#_tc_status_reg
 *
 * PENC Encoder Status Register
 * This register reports status from the internal encoder core.
 */
union cavm_pencx_abx_tc_status_reg
{
    uint64_t u;
    struct cavm_pencx_abx_tc_status_reg_s
    {
#if __BYTE_ORDER == __BIG_ENDIAN /* Word 0 - Big Endian */
    ...
    /* struct cavm_pencx_abx_tc_status_reg_s cn; */
};
typedef union cavm_pencx_abx_tc_status_reg cavm_pencx_abx_tc_status_reg_t;
...
#define typedef_CAVM_PENCX_ABX_TC_STATUS_REG(a,b) cavm_pencx_abx_tc_status_reg_t
#define bustype_CAVM_PENCX_ABX_TC_STATUS_REG(a,b) CSR_TYPE_RSL
#define basename_CAVM_PENCX_ABX_TC_STATUS_REG(a,b) "PENCX_ABX_TC_STATUS_REG"
#define busnum_CAVM_PENCX_ABX_TC_STATUS_REG(a,b) (a)
#define arguments_CAVM_PENCX_ABX_TC_STATUS_REG(a,b) (a),(b),-1,-1
