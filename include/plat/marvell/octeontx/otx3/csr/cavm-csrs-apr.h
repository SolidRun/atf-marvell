 ...
 *
 * PF_FUNC LMTLINE region configuration Structure
 */
union cavm_apr_lmt_map_entry_s
{
    ...
    struct cavm_apr_lmt_map_entry_s_s
    {
        ...
        uint64_t sched_ena             : 1;  /**< [ 86: 86] When clear, Ordered LMTSTs to this PF_FUNC are faulted (CHI NDErr=1). */
        uint64_t dis_line_pref         : 1;  /**< [ 85: 85] When set, disables LMTLINE prefetch before receiving store data. */
        uint64_t lmt_ena               : 1;  /**< [ 84: 84] When clear, LMTSTs to this PF_FUNC are faulted (CHI NDErr=1), */
        uint64_t reserved_83           : 1;
        uint64_t num_lmtlines          : 3;  /**< [ 82: 80] number of LMTLINES at [LMTLINE_BASE] is 2^(5+[NUM_LMTLINES]). 0=32 (CAVM),
                                                                 4=512 (16 cores), 6=2k (64 cores) LMTLINES, other values are
                                                                 reserved (behavior undefined). */
...
union cavm_apr_af_lmt_cfg
{
    ...
    struct cavm_apr_af_lmt_cfg_s
    {
        ...
        uint64_t ignore_poison_pa      : 1;  /**< [ 13: 13](R/W) When set, poison in CompData fetching an LMTLINE's physical address will be
                                                                 recorded, but will not affect transactions issued for the LMTST in any other way. */
        uint64_t sec_lmt               : 1;  /**< [ 12: 12](SR/W) When set, the LMT Map and LMTLINE reqgions all reside in secure memory,
                                                                 otherwise, LMT Map and LMTLINE regions reside in non-secure memory. */
        uint64_t gbl_dis_sched         : 1;  /**< [ 11: 11](R/W) When set disables all Scheduled LMTSTs (CHI NDerr=1 for any attempts). */
        ...
        uint64_t pfs                   : 3;  /**< [  6:  4](R/W) Log base 2 of the number of PFs that are supported, and impacts the size of the
                                                                 LMT Map table. 0=1 PF, 1 = 2 PFs, ..., 5=32 PFs. values greater than 5 are
                                                                 reserved/undefined. */
        uint64_t funcs                 : 4;  /**< [  3:  0](R/W) Log base 2 of the number of FUNCs per PF that are supported, and impacts the
                                                                 size of te LMT Map table. 0=1 Func-per-PF, 4=16 FUNCs-per-PF, 6=64 FUNCs-per-PF,
                                                                 8 = 256 FUNCs-per-PF, other values are reserved/undefined. */
#else /* Word 0 - Little Endian */
        uint64_t funcs                 : 4;  /**< [  3:  0](R/W) Log base 2 of the number of FUNCs per PF that are supported, and impacts the
                                                                 size of te LMT Map table. 0=1 Func-per-PF, 4=16 FUNCs-per-PF, 6=64 FUNCs-per-PF,
                                                                 8 = 256 FUNCs-per-PF, other values are reserved/undefined. */
 ...
 * Base address of LMT map table.
 * Table contains [PFS]x[FUNCS] entries of 16 bytes each (128 KB total), with each
 * entry formatted as APR_LMT_MAP_ENTRY_S.
 * Table is indexed by a (PF, FUNC) extracted from the address of an LMTST.
 * Note that RVU_PF_FUNC_S defines a 16-bit PF_FUNC with 6-bit PF and 10-bit FUNC, but
 * in practice 10xxx addresses only contain a 5-bit PF and 8-bit FUNC.
 * The APR_AF_LMT_CFG[PFS,FUNCS] fields further restrict the bitwidth of the PF and FUNC fields.
 */
