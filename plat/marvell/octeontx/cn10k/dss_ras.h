...
#define MY_DATA_WIDTH  64
#define MY_MAX_WIDTH   (MY_DATA_WIDTH + 8)

// This is for decode of ECCSTAT[corrected_bit_num]
static inline int cn10k_dram_flip2bit(int flip)
{
	int dec, temp;
...
static inline int cn10k_dram_bit2flip(int bit)
{
	int flip;
	 ...
	 */
	for (flip = 0; flip < MY_MAX_WIDTH; flip++)
		if (bit == cn10k_dram_flip2bit(flip))
			return flip;
	return -1;
}
#endif
