...
static void init_mcc(uint64_t config_base, uint64_t config_size)
{
...
struct ecam_init_callback plat_init_callbacks[] = {
	...
	{0xa070, 0x177d, init_mcc}, /* 0x70 - PCC_DEV_IDL_E::MCC */
	{0xa073, 0x177d, init_mdc}, /* 0x73 - PCC_DEV_IDL_E::MDC */
...
static int cn96xx_matched_twsi_slave(int instance)
{
	...
	uint32_t g_pccpf_id, uint32_t g_vsec_ctl)
{
	...
	if (dev->instance == ECAM_CUSTOM_INSTANCE) {
		...
		case ECAM_PROD_DEV_ID(CAVM_PCC_DEV_IDL_E_MIO_TWS):
			return cn96xx_matched_twsi_slave(vsec_ctl.s.inst_num);
		}
	}
