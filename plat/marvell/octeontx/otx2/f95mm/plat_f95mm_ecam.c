...
static void init_cgx(uint64_t config_base, uint64_t config_size)
{
	...
	/* CGX1 is routed to RFOE for F95MM */
	if (cgx_id == 1)
		cgx->is_rfoe = 1;
...
static inline uint64_t f95mm_get_dev_config(struct ecam_device *dev)
{
	uint64_t pconfig;
...
static int f95mm_is_bus_disabled(struct ecam_device *dev)
{
	int rc = 0;
	 ...
	 **********************************************************************/
	/* Below buses does not exist in internal f95mm topology */
...
static int f95mm_skip_bus(struct ecam_device *dev)
{
	return 0;
}

static inline void f95mm_enable_bus(struct ecam_device *dev)
{
...
static inline void f95mm_disable_bus(struct ecam_device *dev)
{
	cavm_ecamx_domx_busx_permit_t bus_permit;
...
static inline void f95mm_enable_dev(struct ecam_device *dev)
{
...
static inline void f95mm_disable_dev(struct ecam_device *dev)
{
	cavm_ecamx_domx_devx_permit_t dev_permit;
...
static inline void f95mm_enable_func(struct ecam_device *dev)
{
...
static inline void f95mm_disable_func(struct ecam_device *dev)
{
	cavm_ecamx_domx_rslx_permit_t rsl_permit;
...
static int f95mm_get_ecam_count(void)
{
...
static int f95mm_get_domain_count(struct ecam_device *dev)
{
	cavm_ecamx_const_t ecam_const;
...
static inline int f95mm_is_domain_present(struct ecam_device *dev)
{
...
static int f95mm_matched_twsi_slave(int instance)
{
...
static int f95mm_matched_dev(struct secure_devices *dev,
	uint32_t g_pccpf_id, uint32_t g_vsec_ctl)
{
	...
	if (dev->instance == ECAM_CUSTOM_INSTANCE) {
		...
		case ECAM_PROD_DEV_ID(CAVM_PCC_DEV_IDL_E_MIO_TWS):
			return f95mm_matched_twsi_slave(vsec_ctl.s.inst_num);
		}
	}
...
static int f95mm_get_secure_settings(struct ecam_device *dev, uint64_t pconfig)
{
	cavm_pccpf_xxx_id_t pccpf_id;
	union cavm_pccpf_xxx_vsec_ctl vsec_ctl;
...
struct ecam_probe_callback *f95mm_get_probe_callbacks(void)
{
	return &probe_callbacks[0];
}

struct ecam_init_callback *f95mm_get_init_callbacks(void)
{
	return &plat_init_callbacks[0];
}

const struct ecam_platform_defs plat_ops = {
	.soc_type = F95MMPARTNUM,
	.get_ecam_count = f95mm_get_ecam_count,
	.get_domain_count = f95mm_get_domain_count,
	.is_domain_present = f95mm_is_domain_present,
	.get_secure_settings = f95mm_get_secure_settings,
	.get_dev_config = f95mm_get_dev_config,
	.get_probes = f95mm_get_probe_callbacks,
	.get_plat_inits = f95mm_get_init_callbacks,
	.is_bus_disabled = f95mm_is_bus_disabled,
	.skip_bus = f95mm_skip_bus,
	.enable_bus = f95mm_enable_bus,
	.disable_bus = f95mm_disable_bus,
	.enable_dev = f95mm_enable_dev,
	.disable_dev = f95mm_disable_dev,
	.enable_func = f95mm_enable_func,
	.disable_func = f95mm_disable_func,
};
