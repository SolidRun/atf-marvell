...
static MZD_STATUS set_serdes_mux_a_rinline1(IN MZD_DEV_PTR pDev)
{
	MZD_STATUS status;
	MZD_U8 serdesMux[MZD_MAX_PORTS*MZD_NUM_LANES] =	{
...
void phy_marvell_7121_probe(int cgx_id, int lmac_id)
{
	...
	if (!strncmp(plat_octeontx_bcfg->bcfg.board_model, "a-rinline1", 10))
		set_serdes_mux_a_rinline1(phy->priv);
