...
#  define debug_cgx_intf(...) (mrvl_tf_log_modules & MRVL_TF_LOG_MODULE) ? \
...
static int cgx_process_requests(int cgx_id, int lmac_id)
{
	...
	scratchx0.u = CSR_READ(CAVM_CGXX_CMRX_SCRATCHX(cgx_id, lmac_id, 0));
	err_type = cgx_get_error_type(cgx_id, lmac_id);
	if (err_type & CGX_ERR_MASK && (request_id != ETH_CMD_GET_LINK_STS))
		scratchx0.s.evt_sts.stat = ETH_STAT_FAIL;
	else
		scratchx0.s.evt_sts.stat = ETH_STAT_SUCCESS;
...
void cgx_set_supported_link_modes(int cgx_id, int lmac_id)
{
			...
			if (!is_gsern)
				lmac_cfg->supported_link_modes |=
					((1 << ETH_MODE_25G_C2C_BIT) |
					 ...
					 (1 << ETH_MODE_25G_CR_BIT) |
					 (1 << ETH_MODE_25G_KR_BIT));
	...
	if ((lmac_cfg->mode == CAVM_CGX_LMAC_TYPES_E_SGMII) ||
		...
		if (is_gsern) {
			modes_allowed = ((1 << ETH_MODE_1000_BASEX_BIT) |
					 (1 << ETH_MODE_SGMII_BIT) |
					 (1 << ETH_MODE_10G_C2C_BIT) |
					 (1 << ETH_MODE_10G_C2M_BIT) |
					 (1 << ETH_MODE_10G_KR_BIT) |
			...
			case QLM_MODE_25G_KR:
				 ...
				 */
				modes_allowed = ((1 << ETH_MODE_25G_CR_BIT) |
						 (1 << ETH_MODE_25G_KR_BIT) |
						 (1 << ETH_MODE_25G_C2C_BIT) |
						 (1 << ETH_MODE_25G_C2M_BIT) |
						 (1 << ETH_MODE_SGMII_BIT) |
