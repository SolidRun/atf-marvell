...
//	eye_state.node = node;
	...
	printf("GSERC%d.%d: Measured TX bit rate %" PRIu64 ".%" PRIu64 " MHz\n", qlm, lane,
 ...
 * @return Zero on success, negative on failure
 */
int qlm_gserc_display_trace(int module, int lane, int type)
{
	uint8_t data[64 * 8];
	int len;
	...
	if (type == 1)
	{
		...
		len = qlm_gserc_get_trace(module, lane, 1, data, sizeof(data));
		decode_an(data, len);
	}
	else
	{
		printf("GSERC%d.%d: Training state machine trace (Local Requests)\n", module, lane);
		len = qlm_gserc_get_trace(module, lane, 2, data, sizeof(data));
		decode_training_state(data, len);
		printf("GSERC%d.%d: Training coefficient trace (Link Partner Requests)\n", module, lane);
...
retry_cmd:
	...
	{
		if (resp_code == 0xFE)
			gser_warn("GSERC%d.%d: AN CFG INST: Invalid Instance Number\n", module, lane);
		else if (resp_code == 0xFD)
			gser_warn("GSERC%d.%d: AN CFG INST: Lane not in Fixed-Rate mode\n", module, lane);
		else
			gser_warn("GSERC%d.%d: AN CFG INST: Unknown response code:%d\n", module, lane, resp_code);
		return -1;
	}
...
retry_cmd:
	...
	if (resp_code != 0)
	{
		if ((cmd_type == AN_CFG_CMD_START) &&
		    (resp_code == 0xfd))
			gser_warn("GSERC%d AN CFG DATA: Lane not in Fixed-Rate mode\n", module);
		else if ((cmd_type == AN_CFG_CMD_CONT) &&
			 (resp_code == 0x1))
			gser_warn("GSERC%d: AN CFG DATA: All AN CFG Instance data received\n", module);
		else
			gser_warn("GSERC%d: AN CFG DATA: Unknown response code: %d\n", module, resp_code);
		return -1;
	}
	return 0;
}
 ...
 */
static void qlm_gserc_config_an_mbox(int module, int lane, ap_802_3_adv_t ap_802_3_adv)
{
	int count = 0;
	uint64_t cmd_arg = 0;
 ...
 */
static ap_802_3_adv_t qlm_gserc_ap_adv(int module, int lane, qlm_802_3ap_fec_t fec_types)
{
	qlm_state_lane_t state = qlm_gserc_get_state(module, lane);
 ...
 */
int qlm_gserc_mode_chg_full_reset(int module, int baud_mhz)
{
	...
	switch (baud_mhz)
	{
		case 25781:
			if ((bcfg.s.phy_ctrl_rate1 != 0x03) ||
			    (bcfg.s.phy_ctrl_refclk != 0x0e)) /* 156.25MHz */
				full_reset_req = 1;
			break;
		case 20625:
		...
		case 4915:
		case 2458:
			if ((bcfg.s.phy_ctrl_rate2 != 0x12) ||
			    (bcfg.s.phy_ctrl_refclk != 0x14)) /* 122.88MHz */
				full_reset_req = 1;
			break;
		case 6250:
		case 3125:
 ...
 * @return Zero on success, negative on failure
 */
int qlm_gserc_cfg_mode(int module, uint8_t lane_mask, qlm_modes_t mode, int baud_mhz,
		       qlm_mode_flags_t flags, int lane_an_master, qlm_802_3ap_fec_t fec_types,
		       qlm_lt_init_state_t lt_init, int ignore_mode_chk)
{
	 ...
	 */
	GSER_CSR_INIT(gsercx_init_ctl, CAVM_GSERCX_COMMON_PHY_CTRL_BCFG(module));
	...
	for (int lane = 0; lane < num_lanes; lane++)
	{
		if (!(lane_mask & (1 << lane)))
			continue;
		GSER_CSR_WRITE(CAVM_GSERCX_SCRATCHX(module, lane), state.u);
	}
	...
	for (int lane = 0; lane < num_lanes; lane++)
	{
		if (!(lane_mask & (1 << lane)))
			continue;
		GSER_CSR_INIT(lane_old, CAVM_GSERCX_LANEX_CONTROL_BCFG(module, lane));
		cavm_gsercx_lanex_control_bcfg_t lane_new = qlm_gserc_get_lane_mode(module, lane);
	...
	GSER_CSR_INIT(bcfg_old, CAVM_GSERCX_COMMON_PHY_CTRL_BCFG(module));
	cavm_gsercx_common_phy_ctrl_bcfg_t bcfg_new = qlm_gserc_get_clock_mode(module);
	...
	for (int lane = 0; lane < num_lanes; lane++)
	{
		...
		if (ln_an_cfg_orig[lane] == QLM_LANE_AN_DIS)
		{
			...
			{
				/* This is an interim step and happens fast, so sometimes we miss it */
				//gser_error("GSERC%d.%d: CHRDY2: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_state_chng_rdy]=0\n", module, lane);
			}
		}
	}
	...
	for (int lane = 0; lane < num_lanes; lane++)
	{
		...
		{
			if (GSER_CSR_WAIT_FOR_FIELD(CAVM_GSERCX_LANEX_STATUS_BSTS(module, lane), GSERCX_STATUS_BSTS_LN_STATE_CHNG_RDY, ==, 1, 10000))
				gser_error("GSERC%d.%d: CHRDY3: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_state_chng_rdy]=1\n", module, lane);
		}
	}
	...
	if (req_state_an)
	{
		...
		for (int lane = 0; lane < num_lanes; lane++)
		{
			...
			{
				ap_802_3_adv_t ap_adv = qlm_gserc_ap_adv(module, lane, fec_types);

				qlm_gserc_config_an_mbox(module, lane, ap_adv);
			}
		}
	...
	for (int lane = 0; lane < num_lanes; lane++)
	{
		...
		if (bcfg.s.ln_an_cfg == QLM_LANE_AN_DIS)
		{
			...
			{
				/* This is an interim step and happens fast, so sometimes we miss it */
				//gser_error("GSERC%d.%d: CHRDY9: Timeout waiting for GSERCX_LANEX_STATUS_BSTS[ln_state_chng_rdy]=0\n", module, lane);
			}
		}
	}
