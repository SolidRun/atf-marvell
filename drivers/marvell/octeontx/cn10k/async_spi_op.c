...
#define SPI_PAGE_ALLIGN (0x111ll)
...
static void op_read(struct delayed_spi_params p)
{
	INFO("%s: Read start: %llx, mem: %llx, size: %llx\n", __func__, p.spi_addr, p.memory_addr, p.size);
	cdns_xspi_direct_op(p.spi_addr, (void *)p.memory_addr, p.size, 0, CDNS_DIRECT_READ);
}

static void op_update(struct delayed_spi_params p)
{
	uint64_t start_page_addr = p.spi_addr & (~SPI_PAGE_ALLIGN);
	uint64_t data_offset = p.spi_addr - start_page_addr;

	INFO("%s: Update start: %llx, mem: %llx, size: %llx\n", __func__, p.spi_addr, p.memory_addr, p.size);
...
static void op_program(struct delayed_spi_params p)
{
	INFO("%s: Prog start: %llx, mem: %llx, size: %llx\n", __func__, p.spi_addr, p.memory_addr, p.size);
	cdns_xspi_direct_op(p.spi_addr, (void *)p.memory_addr, p.size, 0, CDNS_DIRECT_WRITE);
}

static void op_erase(struct delayed_spi_params p)
{
	INFO("%s: Erase start: %llx, cnt: %lld\n", __func__, p.spi_addr, p.erase_block_count);
...
static int async_tim_handler(int tim)
{
	...
	case SPI_OP_ERASE:
		op_erase(spi_ops[spi_op_cnt].op_config);
		break;
	case SPI_OP_PROGRAM:
		op_program(spi_ops[spi_op_cnt].op_config);
		break;
	case SPI_OP_UPDATE:
		op_update(spi_ops[spi_op_cnt].op_config);
		break;
	case SPI_OP_READ:
		op_read(spi_ops[spi_op_cnt].op_config);
		break;
	case SPI_OP_NONE:
...
{
	if (!tim_initialized) {
		timer_hd = timer_create(TM_ONE_SHOT, 10, async_tim_handler);
		tim_initialized = 1;
	}
	if (timer_hd < 0) {
...
{
	uint64_t page_addr = *addr & (~SPI_PAGE_ALLIGN);
...
static void spi_write_delayed(uint64_t addr, uint64_t size, uint64_t buffer)
{
	...
	while (size) {
		if (addr & SPI_PAGE_ALLIGN || size < SPI_ERASE_SIZE) {
 ...
 * @param	cs		spi cs
 * @param	spi_addr	Adress where data will be written
 * @param	mem_addr	Adress of memory buffer
 ...
 * @param[in]	bus		spi bus
 * @param	cs		spi cs
 * @param	spi_addr	Adress from where data will be read
 * @param	mem_addr	Adress of memory buffer
