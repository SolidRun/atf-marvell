					...
					rpm_link_state_t *link, int err_type)
{
	...
	if (lmac_cfg->phy_present && lmac_cfg->phy_config)
		...
		if (cn10k_lpcs_an_disable_mode(portm->portm_mode) > 0)
			    ...
			    int req_fec, uint64_t *eth_flash_update_req_mask)
{
	...
	if (!ret) {
		if ((req_fec == PORTM_FEC_RS) &&
				((fec == PORTM_FEC_RS_528_ONLY) || (fec == PORTM_FEC_RS_544_ONLY))) {
			debug_rpm_intf("%s: %d:%d: FEC type %s supported for this mode %s is enabled default\n",
			       __func__, rpm_id, lmac_id,
			       cn10k_portm_fec_type_to_str(req_fec),
			       cn10k_portm_mode_to_cfg_str(portm->portm_mode));
			portm->fec = lmac_ctx->s.fec = req_fec;
		} else
				      ...
				      uint64_t *eth_flash_update_req_mask)
{
	...
	if (portm->portm_mode == portm_mode) {
		if ((lmac->mode == CAVM_RPM_LMAC_TYPES_E_USGMII) ||
			(lmac->mode == CAVM_RPM_LMAC_TYPES_E_USXGMII) ||
			(cn10k_lpcs_an_disable_mode(portm->portm_mode) > 0)) {
