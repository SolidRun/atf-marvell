 ...
 *
 */
void fill_portm_tx_eq_info(int portm_idx, portm_modes_t portm_mode)
{
	void *fdt = fdt_ptr;
	int offset, rc;
	char prop[64];
	...
	const char *portm_mode_str = portm_mode_to_cfg_str(portm_mode);
	...
	for (int lane = 0; lane < portm->gser_numlanes; lane++) {
		...
		if (!portm_tx_tuning_valid(portm_idx, lane, &tx_tuning)) {
 ...
 *
 */
void fill_portm_lane_map(int portm_idx)
{
	portm_config_t *portm;
 ...
 *
 */
void fill_portm_mac_info(int portm_idx, portm_modes_t portm_mode)
{
	...
	if (mac_type == PORTM_ETH) {
		portm->mac_num = portm_get_rpm_num(portm_idx);
		portm->mac_lane = portm_get_rpm_lmac_num(portm_idx);
		portm->pcs_type = portm_get_pcs_type(portm_mode);
	} else {
		portm->mac_num = portm_get_other_mac_num(portm_idx);
		portm->mac_lane = portm_get_other_mac_lane_num(portm_idx);
	}
 ...
 *
 */
void fill_portms_used(int portm_idx, portm_modes_t portm_mode)
{
	portm_config_t *portm;
 ...
 */
int portm_802_3ap_cfg(int portm_idx, portm_ap_802_3_config_t *ap_802_3_cfg,
			    portm_modes_t *portm_mode, int *numlanes)
{
	...
	for (int cfg = 0; cfg < PORTM_MAX_AN_CFGS; cfg++) {
		...
		*numlanes = portm_get_mode_desc_serdes_num(ap_mode);
		if (*numlanes >= numlanes_max) {
			numlanes_max = *numlanes;
			speed = portm_get_mode_desc_speed_mhz(ap_mode);
			if (speed > speed_max) {
				speed_max = speed;
		...
		portm_update_802_3ap_adv(ap_mode, fec_req, fec_abil, &ap_adv_temp);
