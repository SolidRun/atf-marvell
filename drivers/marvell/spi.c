...
static int __spi_secure_validate_img(uintptr_t user_buf, uintptr_t size,
...
static int __spi_secure_update_img(uintptr_t user_buffer, uintptr_t size,
					uint32_t bus, uint32_t cs)
{
	...
	while (size > 0) {
		...
		if (spi_nor_erase(offset, mode, bus, cs)) {
			debug_spi_nor("Secure SPI: Erase flash failed\n");
			ret = -1;
			break;
		}
		...
		if (spi_nor_write((uint8_t *)wr_buffer, BUF_SIZE, offset,
				    mode, bus, cs) < 0) {
			debug_spi_nor("Secure SPI: Write flash failed\n");
			ret = -1;
			break;
		}
		if (spi_nor_read((uint8_t *) rd_buffer, BUF_SIZE, offset,
					mode, bus, cs) < 0) {
			debug_spi_nor("Secure SPI: Read flash failed\n");
			ret = -1;
			break;
		}
#ifdef DEBUG_SPI_NOR
		...
		if (memcmp((const void *)rd_buffer,
			...
			debug_spi_nor("Secure SPI: Compare data failed\n");
			ret = -1;
			break;
		}
				...
				uint32_t bus, uint32_t cs)
{
	...
	debug_spi_nor("Secure SPI: bus:0x%llx cs:0x%llx\n", bus, cs);

	/* Init Secure SPI */
	/* FIXME */
	/* Need to parse FDT to config SPI */
	if (spi_config(CONFIG_SPI_FREQUENCY, 0, 0, 0, bus, cs)) {
		debug_spi_nor("Secure SPI: Config flash failed\n");
		return -SPI_CONFIG_ERR;
	}

	ns_map_size = size < PAGE_SIZE?PAGE_SIZE:(((size/PAGE_SIZE)+1)*PAGE_SIZE);
	...
	if (octeontx_mmap_add_dynamic_region_with_sync(user_buf,
		...
		ns_map_size,
		MT_RW | MT_NS)) {
		debug_spi_nor("Secure SPI: mmap failed (%d)\n", err);
		return -SPI_MMAP_ERR;
	}

	/* Validate image before update */
	if (__spi_secure_validate_img(user_buf, size, bus, cs)) {
		debug_spi_nor("Secure SPI: Update image validation failed\n");
		err = -SPI_IMG_VALIDATE_ERR;
		goto err;
	}

	/* Update image */
	if (__spi_secure_update_img(user_buf, size, bus, cs)) {
