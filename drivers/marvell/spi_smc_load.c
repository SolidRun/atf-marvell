...
/* Read, write, erase of secure SPI-NOR is only allowed for certain ranges
 * Check the range being operated on before doing it.
 */
			...
			int loc, int bus, int cs)
{
	...
	/* Check of device is secure */
	if (!plat_octeontx_bcfg->spi_cfg[bus].is_secure) {
		*efi_size = 0;
		return -1;
	}

	/* Check read range */
	if ((loc < OEM_DATA_OFFSET) || (loc > (OEM_DATA_OFFSET + OEM_DATA_SIZE)) ||
	   (loc + size > OEM_DATA_OFFSET + OEM_DATA_SIZE)) {
		*efi_size = 0;
		return -1;
	}
...
int spi_smc_secure_write(uintptr_t efi_buf, uint64_t *efi_size,
			 int loc, int bus, int cs)
{
	/* Write to secure flash not allowed */
	WARN("Secure SPI: Write not supported\n");
	*efi_size = 0;
	return -1;
}

int spi_smc_secure_erase(uint64_t *efi_size, int loc, int bus, int cs)
{
	/* Erase of secure flash not allowed */
	WARN("Secure SPI: Erase not supported\n");
	*efi_size = 0;
	return -1;
}
